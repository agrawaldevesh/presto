        ...
        implements RowExpressionVisitor<PinotExpression, Map<VariableReferenceExpression, Selection>>
{
    ...
    public PinotFilterExpressionConverter(TypeManager typeManager, FunctionMetadataManager functionMetadataManager)
    {
    ...
    private PinotExpression handleFunction(CallExpression function, Map<VariableReferenceExpression, Selection> context)
    {
        ...
        throw new PinotException(PINOT_UNSUPPORTED_EXPRESSION, Optional.empty(), format("function %s not supported in filter", function));
    }

    private PinotExpression handleLogicalBinary(String operator, CallExpression call, Map<VariableReferenceExpression, Selection> context)
    {
    ...
    private PinotExpression handleBetween(CallExpression between, Map<VariableReferenceExpression, Selection> context)
    {
    ...
    private PinotExpression handleNot(CallExpression not, Map<VariableReferenceExpression, Selection> context)
    {
    ...
    public PinotExpression visitCall(CallExpression call, Map<VariableReferenceExpression, Selection> context)
    {
        ...
        if (displayName.equals("not")) {
            return handleNot(call, context);
        }
        else if (displayName.equals("cast")) {
            return handleCast(call, context);
        }
        else if (displayName.equals("between")) {
    ...
    private PinotExpression handleIn(SpecialFormExpression specialForm, boolean isWhitelist, Map<VariableReferenceExpression, Selection> context)
    {
        return derived(format("(%s %s (%s))",
